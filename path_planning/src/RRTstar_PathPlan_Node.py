...
class RRTstar_PathPlan_Node:
    ...
    def __init__(self):
        ...
        if rospy.has_param('~odom_topic'):
          ...
          #  self.odometry_topic = "/odometry"
            self.odometry_topic = "/robot_control/odom"
    ...
    def sampleTree(self):
        ...
        for cone in frontCones:
            #coneObstacleList.append((cone[0], cone[1], coneObstacleSize))
            coneObstacleList.append((cone.x, cone.y, coneObstacleSize))

            #coneDist = self.dist(self.carPosX, self.carPosY, cone[0], cone[1])
            coneDist = self.dist(self.carPosX, self.carPosY, cone.x, cone.y)            

            if coneDist > frontConesDist * coneTargetsDistRatio:
                #rrtConeTargets.append((cone[0], cone[1], coneObstacleSize))
        ...
        rrt = rrt_star.RRT(start, planDistance, obstacleList=coneObstacleList, expandDis=expandDistance, turnAngle=expandAngle, maxIter=iterationNumber, rrtTargets = rrtConeTargets)
    ...
    def findBestBranch(self, leafNodes, nodeList, largerGroupFrontCones, coneObstacleSize, expandDistance, planDistance):
        ...
        coneDistLimit = 4.0
        coneDistanceLimitSq = coneDistLimit * coneDistLimit  # semicolon tha yahan par. if u want then put it back
        ...
        for leaf in leafNodes:
            ...
            while node.parent is not None:
                ...
                for cone in largerGroupFrontCones:
                    #coneDistSq = ((cone[0] - node.x) ** 2 + (cone[1] - node.y) ** 2)
    ...
    def isLeftCone(self, node, parentNode, cone):
        ...
        #return ((node.x - parentNode.x) * (cone[1] - parentNode.y) - (node.y - parentNode.y) * (cone[0] - parentNode.x)) > 0;
    ...
    def getFrontConeObstacles(self, map, frontDist):
        ...
        frontConeList = []
        for cone in map:
            #if (headingVectorOrt[0] * (cone[1] - carPosBehindPoint[1]) - headingVectorOrt[1] * (cone[0] - carPosBehindPoint[0])) < 0:
            if (headingVectorOrt[0] * (cone.y - carPosBehindPoint[1]) - headingVectorOrt[1] * (cone.x - carPosBehindPoint[0])) < 0:    
                #if ((cone[0] - self.carPosX) ** 2 + (cone[1] - self.carPosY) ** 2) < frontDistSq:
    ...
    def getConesInRadius(self, map, x, y, radius):
        ...
        for cone in map:
            ...
            #if ((cone[0] - x) ** 2 + (cone[1] - y) ** 2) < radiusSq:
